...
class TestSet(Dataset):
    ...
    def __getitem__(self, idx):
        image, metadata, _ = super().__getitem__(idx)
...
def create_test_set(data_path, csv_path, mean_age=48.9, std_age=14.4,
                    transform=None, balance=False, sample=1.0):
    image_path = []
    metadata = []
    anatom_category = defs.metadata_anatom_categories
    ...
    # Create dataset.
    dummy_target = [0]*len(image_path)
    dataset = TestSet(image_path, metadata, dummy_target, transform, balance)
    return dataset
